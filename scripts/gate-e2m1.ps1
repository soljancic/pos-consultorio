...
Invoke-RestMethod -Uri "$base/cobros/$($cobro.id)/pagos" -Method Post -Headers $hSec -ContentType "application/json" -Body (@{ monto = 1000; tipoCuentaId = $tcQr } | ConvertTo-Json) | Out-Null
...
Invoke-RestMethod -Uri "$base/cobros/$($cobro.id)/pagos" -Method Post -Headers $h -ContentType "application/json" -Body (@{ monto = $cobroAct.saldoPendiente; tipoCuentaId = $tcEfectivo } | ConvertTo-Json) | Out-Null
