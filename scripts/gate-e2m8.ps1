...
$tgServicios = ($tiposGasto | Where-Object { $_.nombre -eq 'Servicios' }).id
...
Invoke-RestMethod -Uri "$base/gastos" -Method Post -Headers $h -ContentType "application/json" -Body (@{ fecha = $hoy; tipoGastoId = $tgServicios; monto = 300; descripcion = "internet"; tipoCuentaId = $tcBanco } | ConvertTo-Json) | Out-Null
...
# 3) Validacion DTO
Esperar-Error { Invoke-RestMethod -Uri "$base/gastos" -Method Post -Headers $h -ContentType "application/json" -Body (@{ fecha = $hoy; monto = 100; descripcion = "x"; tipoCuentaId = $tcBanco } | ConvertTo-Json) } 400 "3 SIN TIPO GASTO"
...
$gSec = Invoke-RestMethod -Uri "$base/gastos" -Method Post -Headers $hSec -ContentType "application/json" -Body (@{ fecha = $hoy; tipoGastoId = $tgOtros; monto = 50; descripcion = "taxi"; tipoCuentaId = $tcOtro } | ConvertTo-Json)
